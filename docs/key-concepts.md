...
Orama uses a **Layers of Orchestration** model to drive it's scenes. A `Scene` orchestrates `Entity` objects, and each `Entity` orchestrates it's attached `Component` objects. This layering hierarchy drives how logic is written.
